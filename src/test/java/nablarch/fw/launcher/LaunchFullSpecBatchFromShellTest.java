...
public class LaunchFullSpecBatchFromShellTest {
    ...
    private void createConfigFile() {
        // アプリケーション構成
        Hereis.file(new File(temporaryFolder.getRoot(), "batch-config.xml").getAbsolutePath());
        /***********************************************************************
    ...
    public void testNormalEnd() throws Exception {
        String configFilePath = new File(temporaryFolder.getRoot(), "batch-config.xml").getAbsolutePath();
        ...
        for (ResultData result : resultList) {
            if ("action.createReader".equals(result.activity)) {
                actionCreateReader++;
            }
            if ("action.handle".equals(result.activity)) {
                actionHandle++;
            }
            if ("action.transactionSuccess".equals(result.activity)) {
                actionTransactionSuccess++;
            }
            if ("action.transactionFailure".equals(result.activity)) {
                actionTransactionFailure++;
            }
            if ("action.initialize".equals(result.activity)) {
                actionInitialize++;
            }
            if ("action.error".equals(result.activity)) {
                actionError++;
            }
            if ("action.terminate".equals(result.activity)) {
                actionTerminate++;
            }
            if ("beforelog.handle".equals(result.activity)) {
                beforelogHandle++;
            }
            if ("afterlog.handle".equals(result.activity)) {
                afterlogHandle++;
            }
        }
